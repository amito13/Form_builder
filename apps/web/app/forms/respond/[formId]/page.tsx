...
export default function PublicFormPage() {
  ...
  const submitForm = trpc.form.submitPublicForm.useMutation({
  ...
  if (formQuery.isPending)
    return (
      <PublicFeedback
        title="Loading form…"
        detail="One moment while we prepare it."
      />
    );
  if (formQuery.isError || !formQuery.data)
    return (
      <PublicFeedback
        ...
        detail={
          formQuery.error?.message || "This form may no longer be available."
        }
      />
    );
    ...
    <main className="public-form-page">
      <section className="public-form-shell">
        <header className="public-form-heading">
          <span className="forms-brand-mark" aria-hidden="true">
            F
          </span>
          <p className="forms-section-label">Shared with you</p>
          <h1>{form.title}</h1>
        ...
        {form.fields.length === 0 ? (
          <PublicFeedback
            ...
            detail="The form owner has not added any questions."
            embedded
          />
        ) : (
          <form className="public-form-card" onSubmit={submit}>
            ...
            {submitForm.error && (
              <p className="new-form-error" role="alert">
                {submitForm.error.message || "Unable to submit your response."}
              </p>
            )}

            <button
              className="btn-primary public-form-submit"
              type="submit"
              disabled={submitForm.isPending}
            >
              {submitForm.isPending ? "Sending response…" : "Submit response"}{" "}
...
function PublicFeedback({ title, detail, embedded = false }: { title: string; detail: string; embedded?: boolean }) {
  const content = (
    <section className="public-form-feedback">
      <span className="forms-brand-mark" aria-hidden="true">
        F
      </span>
      <h1>{title}</h1>
