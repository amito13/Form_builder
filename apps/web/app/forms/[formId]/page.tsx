...
export default function FormBuilderPage() {
  ...
  const formQuery = trpc.form.getFormByShareToken.useQuery(
    { shareToken },
  ...
  const refreshForm = () =>
    utils.form.getFormByShareToken.invalidate({ shareToken });
  ...
  if (!shareToken)
    return (
      <BuilderFeedback
        ...
        detail="Return to your workspace and choose a form to edit."
      />
    );
  if (formQuery.isPending)
    return (
      <BuilderFeedback
        title="Loading form…"
        detail="Getting your form ready to edit."
      />
    );
  if (formQuery.isError || !formQuery.data)
    return (
      <BuilderFeedback
        ...
        detail={formQuery.error?.message || "This form could not be found."}
      />
    );
    ...
    <main className="builder-page">
      <header className="builder-header">
        ...
        <div className="builder-heading">
          <div>
            <p className="forms-section-label">Form builder</p>
            <h1>{form.title}</h1>
            <p>{form.description || "Add fields to begin collecting responses."}</p>
          </div>

          <div className="builder-heading-actions">
            <button
              type="button"
              className="btn-secondary builder-share-button"
              onClick={copyShareLink}
            >
              Share form <span aria-hidden="true">↗</span>
            </button>

            <button
              type="button"
              className="btn-ghost builder-responses-button"
              onClick={() => setShowResponses((current) => !current)}
            >
            ...
            <button
              type="button"
              className="builder-delete-form-button"
            ...
            <span className="builder-field-count">
        ...
        {shareMessage && (
          <p className="builder-share-message" role="status">
            {shareMessage}
          </p>
        )}
      </header>
      ...
      <section className="builder-layout">
          ...
          <form onSubmit={addField} className="builder-add-form">
            <label>
              ...
              <input
                ...
                maxLength={100}
                placeholder="e.g. What should we improve?"
                required
              />
            </label>
        ...
        <section className="builder-fields" aria-labelledby="builder-fields-title">
          <div className="builder-fields-heading">
            <div>
              <p className="forms-section-label">Fields</p>
          ...
          {mutationError && (
            <p className="new-form-error" role="alert">
              {mutationError.message || "Unable to save the field."}
            </p>
          )}
          ...
          {form.fields.length === 0 ? (
            <div className="builder-empty">
              ...
              <p>Add the first field from the panel to the left.</p>
            </div>
          ) : (
            <div className="builder-field-list">
              {form.fields.map((field: typeof form.fields[number], index: number) => (
                <EditableField
                  key={String(field.id)}
                  field={field}
                  number={index + 1}
                  ...
                  onDelete={() =>
                    deleteField.mutate({ fieldId: Number(field.id) })
                  }
                />
              ))}
            </div>
          ...
          {showResponses && (
            <ResponsesPanel
              fields={form.fields}
              isLoading={responsesQuery.isPending}
              error={responsesQuery.error?.message}
...
function ResponsesPanel({
  ...
  fields: Array<{ id: unknown; label: string; type: FieldType }>;
  isLoading: boolean;
  error?: string;
  responses?: Array<{
    id?: unknown;
    createdAt: Date | string | null;
    values: Array<{ formFieldId: number; value: string }> | null;
  }>;
}) {
  const fieldsById = new Map(fields.map((field) => [Number(field.id), field]));

  return (
    <section className="builder-responses" aria-labelledby="responses-title">
      <div>
        <p className="forms-section-label">Responses</p>
        <h2 id="responses-title">Submitted answers</h2>
      </div>

      {isLoading && <p className="builder-responses-note">Loading responses…</p>}
      {error && (
        <p className="new-form-error" role="alert">
          {error}
        </p>
      ...
      {!isLoading && !error && responses?.length === 0 && (
        <p className="builder-responses-note">
          No responses have been submitted yet.
        </p>
      )}

      {responses?.map((response, index) => (
        <article key={String(response.id)} className="builder-response-card">
          <header>
            ...
            {response.values?.map((answer) => {
              ...
              return (
                <div key={answer.formFieldId}>
                  <dt>{field?.label || "Deleted field"}</dt>
                  <dd>{field?.type === "PASSWORD" ? "••••••••" : answer.value || "—"}</dd>
                </div>
              );
            })}
          </dl>
...
function EditableField({ field, number, isSaving, onSave, onDelete }: EditableFieldProps) {
  ...
  return (
    <article className="builder-field-card">
      <div className="builder-field-topline">
      ...
      {isEditing ? (
        ...
        <div className="builder-field-preview">
          <h3>
            {field.label}
            {field.isRequired && <span aria-label="Required"> *</span>}
          </h3>

          {field.description && <p>{field.description}</p>}

          <div className="builder-input-preview">
            {field.type === "YES_NO" ? (
              <span>Yes / No</span>
            ) : (
              field.placeholder ||
              `Answer as ${fieldTypeLabels[field.type].toLowerCase()}`
            )}
          </div>
...
function BuilderFeedback({ title, detail }: { title: string; detail: string }) {
  return (
    <main className="builder-feedback-page">
      <section>
        <Link href="/forms" className="new-form-back">
          ← All forms
        </Link>
